...
function tokenize(src: string): Token[] {
  ...
  while (i < src.length) {
    ...
    // Multi-char tokens
    if (src.substring(i, i + 3) === '<:>') {
      tokens.push({ type: 'PLAY', value: '<:>', line });
      i += 3;
      continue;
    }
    if (src.substring(i, i + 3) === '<!>') {
      tokens.push({ type: 'WRITE', value: '<!>', line });
      i += 3;
      continue;
    }
    if (src.substring(i, i + 3) === '<?>') {
      tokens.push({ type: 'READ', value: '<?>', line });
      i += 3;
      continue;
    }
    if (src.substring(i, i + 2) === '|:') {
      tokens.push({ type: 'PIPE_COLON', value: '|:', line });
      i += 2;
      continue;
    }
    if (src.substring(i, i + 2) === ':|') {
      tokens.push({ type: 'COLON_PIPE', value: ':|', line });
      i += 2;
      continue;
    }
    if (src.substring(i, i + 2) === '<-') {
    ...
    if (src.substring(i, i + 2) === '<<') {
      tokens.push({ type: 'APPEND', value: '<<', line });
      i += 2;
      continue;
    }
    if (src.substring(i, i + 2) === '8<') {
      tokens.push({ type: 'CUT', value: '8<', line });
      i += 2;
      continue;
    }
    if (src.substring(i, i + 2) === '/=') {
    ...
    // Single-char tokens
    if (src[i] === '(') {
      tokens.push({ type: 'LPAR', value: '(', line });
      i++;
      continue;
    }
    if (src[i] === ')') {
      tokens.push({ type: 'RPAR', value: ')', line });
      i++;
      continue;
    }
    if (src[i] === '{') {
    ...
    if ('=<>'.includes(src[i])) {
      tokens.push({ type: 'CMP', value: src[i], line });
      i++;
      continue;
    }
    if ('+-*/%^'.includes(src[i])) {
      tokens.push({ type: 'OP', value: src[i], line });
      i++;
      continue;
    }
    ...
    if (/[a-zA-Z]/.test(src[i])) {
      ...
      if (word === 'if') {
        tokens.push({ type: 'IF', value: word, line });
      } else if (word === 'else') {
        tokens.push({ type: 'ELSE', value: word, line });
      } else if (word === 'while') {
        tokens.push({ type: 'WHILE', value: word, line });
      } else if (word in NOTE_TO_INT) {
        tokens.push({ type: 'NOTE', value: word, line });
      } else if (/^[A-Z]/.test(word)) {
        tokens.push({ type: 'FUNC_ID', value: word, line });
      } else {
        tokens.push({ type: 'VAR_ID', value: word, line });
      }
...
class Parser {
  ...
  parseAtom(): ASTNode {
    ...
    if (t.type === 'NUM') {
      this.advance();
      return { type: 'num', value: parseInt(t.value) };
    }
    if (t.type === 'NOTE') {
      this.advance();
      return { type: 'num', value: NOTE_TO_INT[t.value] };
    }
    if (t.type === 'HASH') {
      this.advance();
      return { type: 'list_size', name: t.value };
    }
...
export function playNotes(
  ...
  return new Promise((resolve) => {
    ...
    noteInts.forEach((n, i) => {
      ...
      if (onTick) {
        setTimeout(
          () => onTick({ pitch: n, freq, idx: i, dur: beatDuration }),
          i * beatDuration * 1000
        );
      }
    });
