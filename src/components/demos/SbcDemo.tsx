...
interface Prefs {
  ...
  daysMin: number;
  daysMax: number;
  daysPerCityMin: number;
  daysPerCityMax: number;
  citiesMin: number;
  citiesMax: number;
  budget: number;
...
function planTrip(prefs: Prefs): TripResult {
  ...
  let totalDays = 0;
  const maxCities =
    prefs.priority === 'cities' ? prefs.citiesMax : Math.min(prefs.citiesMax, prefs.citiesMin + 1);
  ...
  for (const cp of plans) {
    const hotels = ACCOMMODATIONS.filter(
      (a) =>
        a.cityId === cp.city.id &&
        a.stars >= prefs.minStars &&
        a.pricePerNight * cp.days <= budgetLeft
    ).sort((a, b) => a.pricePerNight - b.pricePerNight);
    const hotel =
      hotels[0] ??
      ACCOMMODATIONS.filter(
        (a) => a.cityId === cp.city.id && a.pricePerNight * cp.days <= budgetLeft
  ...
  for (let i = 0; i < plans.length - 1; i++) {
    ...
    let routes = TRANSPORTS.filter(
      (t) =>
        t.fromCityId === plans[i].city.id &&
        t.toCityId === plans[i + 1].city.id &&
        !avoidLower.includes(t.mode.toLowerCase()) &&
        t.price <= budgetLeft
    );
    if (!routes.length) {
  ...
  if (totalDays < prefs.daysMin)
    errors.push(`Trip too short: ${totalDays} days (min ${prefs.daysMin})`);
  if (plans.length < prefs.citiesMin)
    errors.push(`Too few cities: ${plans.length} (min ${prefs.citiesMin})`);
      ...
      case 1:
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {ages.map((age, i) => (
              <div key={i} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <input
                  ...
                  onChange={(e) => {
                    const n = [...ages];
                    n[i] = Math.max(0, Math.min(120, Number(e.target.value)));
                    setAges(n);
                  }}
                />
                {ages.length > 1 && (
                  <button
                    ...
                    onClick={() => setAges(ages.filter((_, j) => j !== i))}
                  >
                    {t.removeAge}
                  </button>
                )}
            ...
            <button
              style={{ ...s.btn, ...s.secondaryBtn, alignSelf: 'flex-start' }}
              onClick={() => setAges([...ages, 30])}
            >
              {t.addAge}
            </button>
          </div>
    ...
    return (
      <div>
        ...
        <div style={s.summary}>
          ...
          <div style={s.summaryItem}>
            ...
            <div style={s.summaryLabel}>
              {lang === 'en' ? 'Cities' : lang === 'es' ? 'Ciudades' : 'Ciutats'}
            </div>
          </div>
          <div style={s.summaryItem}>
        ...
        <div style={{ ...s.navRow, justifyContent: 'center' }}>
          <button style={{ ...s.btn, ...s.primaryBtn }} onClick={reset}>
            {t.newTrip}
          </button>
        </div>
      </div>
    ...
    <div style={s.wrapper}>
      ...
      {!result ? (
        <div style={s.card}>
          ...
          <div style={s.navRow}>
            {step > 1 ? (
              <button style={{ ...s.btn, ...s.secondaryBtn }} onClick={goBack}>
                {t.back}
              </button>
            ) : (
              <div />
            ...
            {step < 10 ? (
              <button style={{ ...s.btn, ...s.primaryBtn }} onClick={goNext}>
                {t.next}
              </button>
            ) : (
              <button style={{ ...s.btn, ...s.primaryBtn }} onClick={doPlan}>
                {t.plan}
              </button>
            )}
          </div>
