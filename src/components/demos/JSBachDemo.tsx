...
function JSBachDemo({ lang = 'en' }: { lang?: Lang }) {
  ...
  const play = useCallback(async () => {
    ...
    result.notes.forEach((_, i) => {
      playbackTimeoutsRef.current.push(setTimeout(() => setActiveNote(i), i * beatMs));
    });
        ...
        <div style={styles.sampleBtns}>
          {SAMPLE_PROGRAMS.map((prog) => (
            <button
              ...
              onClick={() => {
                log.info('sample', { name: prog.name });
                setCode(prog.code);
                setResult(null);
                setActiveNote(-1);
              }}
        ...
        <div style={styles.card}>
          ...
          {result.notes.length > 0 && (
            <>
              ...
              >
                {t.notesGenerated
                  .replace('{0}', String(result.notes.length))
                  .replace('{s}', result.notes.length !== 1 ? 's' : '')}
              </div>
          ...
          {!result.error && result.output.length === 0 && result.notes.length === 0 && (
            <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{t.noOutput}</div>
          )}
        </div>
      )}
    </div>
