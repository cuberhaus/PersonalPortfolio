    ...
    fetch(`${BACKEND_URL}/api/news/`)
      ...
      .then((data) => {
        ...
        const items = Object.values(
          data.news as Record<string, { title: string; link: string; img: string }>
        );
        setNews(items);
        setBackendUp(true);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' as const, height: '400px' }}>
      ...
      >
        ...
        {loading && (
          <div
            style={{ padding: '0.5rem 0.75rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}
          >
            {t.typing}
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: '0.5rem' }}>
        ...
        />
        <button
          style={{ ...s.btn, ...s.primaryBtn, whiteSpace: 'nowrap' as const }}
          onClick={send}
          disabled={loading}
        >
          {t.send}
...
function QuizTab({ t }: { t: typeof TRANSLATIONS.en }) {
  ...
  const result =
    band === 'mild' ? t.quizResult1 : band === 'moderate' ? t.quizResult2 : t.quizResult3;
  const resultColor =
    band === 'mild'
      ? 'var(--accent-start)'
      : band === 'moderate'
    ...
    <div style={{ ...s.card, textAlign: 'center' as const, maxWidth: '500px', margin: '0 auto' }}>
      ...
      {done ? (
        <>
          ...
          <button style={{ ...s.btn, ...s.primaryBtn }} onClick={restart}>
            {t.restart}
          </button>
        </>
      ) : (
        <>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
            ...
            <button
              style={{ ...s.btn, ...s.secondaryBtn, minWidth: '80px' }}
          ...
          {Array.from({ length: 30 }, (_, i) => i + 1).map((day) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isPeriod
                    ? 'color-mix(in srgb, var(--accent-end) 20%, transparent)'
                    : 'var(--bg-secondary)',
                  border: isPeriod
