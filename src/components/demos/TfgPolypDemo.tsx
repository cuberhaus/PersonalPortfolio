...
function ModelComparison({ t }: { t: typeof TRANSLATIONS.en }) {
  ...
  const maxVal = useMemo(
    () => Math.max(...sorted.map((m) => (m as any)[metric]), 0.01),
    [sorted, metric]
  );
        ...
        {(['f1', 'ap50', 'ap5095', 'ar100'] as const).map((m) => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              border:
                metric === m ? '1px solid var(--accent-start)' : '1px solid var(--border-color)',
              background:
                metric === m
        ...
        {(['metric', 'lr', 'epochs'] as const).map((s) => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              border: 'none',
              background: sortBy === s ? 'var(--border-color)' : 'transparent',
  ...
  return (
    <div>
      ...
      >
        ...
        <br />
        {t.simDesc2}
        <strong style={{ color: 'var(--text-primary)' }}>{t.simDesc3}</strong>
        {t.simDesc4}
      </p>
...
function CycleGanVisualizer({ t }: { t: typeof TRANSLATIONS.en }) {
  ...
  return (
    <div>
      ...
      >
        {t.cgDesc1}
        <strong style={{ color: 'var(--text-primary)' }}>{t.cgDesc2}</strong>
        {t.cgDesc3}
      </p>
          ...
          <svg viewBox="0 0 200 160" style={{ width: '100%', height: 'auto', display: 'block' }}>
            {direction === 'mask2polyp' ? (
              <>
                ...
                <text x="100" y="150" textAnchor="middle" fill="var(--text-muted)" fontSize="10">
                  {t.binMask}
                </text>
              </>
            ) : (
      ...
      >
        ...
        >
          <div>
            ...
            >
              {t.dashboard1}
              <strong style={{ color: 'var(--text-secondary)' }}>{t.dashboard2}</strong>
              {t.dashboard3}
              <code style={{ color: 'var(--text-muted)', fontSize: '0.72rem' }}>
                docker compose up
              </code>
              {t.dashboard4}
            </p>
          ...
          <div style={{ marginTop: '1.25rem' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: 'var(--text-primary)' }}>
              Pol Casacuberta &middot; FIB-UPC
            </div>
          </div>
        </div>
      </div>
