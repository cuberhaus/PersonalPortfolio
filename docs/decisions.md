# Observability decisions — research catalogue

Research record behind the chosen observability stack: foundations evaluated,
...
> traceable history of **what was considered and rejected**. Anchor links
> from outside (e.g. `src/lib/debug-sentry.ts`'s "Concept mapping"
> reference) now point here.
...
The architecture doc summarises **what** was chosen; this section keeps the
**why** for the long tail of "what about X?" questions.
...
pattern-decision (event bus) is committed up front; foundation (custom) and
backend (Sentry hosted) are interchangeable downstream.
