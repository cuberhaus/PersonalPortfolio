...
- **CV download has two modes** — `About.astro` uses [`CvDownloader.tsx`](src/components/CvDownloader.tsx) (preset + photo mode) when `public/cv/cv_english_standard_photo.pdf` exists (i.e. `deploy.yml` has fetched the curated variants from cuberhaus/cv). Falls back to a language-only download when only `public/cv.pdf` / `cv_es.pdf` / `cv_ca.pdf` are present locally. To exercise the selector locally, create a stub `public/cv/cv_english_standard_photo.pdf` and run `npm run dev`.
